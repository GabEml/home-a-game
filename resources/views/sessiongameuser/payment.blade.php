            ...
            <form action="{{ route('sessiongameusers.storePayment') }}" method="post" class="" id="form">
                ...
                <fieldset>
                        ...
                        <table class="table-bordered table-striped table-hover align-middle table tableGoodie">
                            ...
                            <tbody>
                                @foreach ($sessiongames as $sessiongame)
                                    ...
                                    <tr>
                                        <td>
                                            <input class="form-check-input" type="hidden" value="{{ $sessiongame->id }}"
                                                name="sessiongames[]" class=@error('session') is-invalid @enderror>
                                            <label class="form-check-label" for="flexCheckDefault">
                                                {{ $sessiongame->name }} du {{ $sessiongame->start_date }} au
                                                {{ $sessiongame->end_date }}
                                            </label>
                                        </td>
                                        <td class="text-right">
                                            {{ $sessiongame->price }} €
                    ...
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" value="1" required name="conditions" id="flexCheckDefault">
                        <label class="form-check-label" for="flexCheckDefault">
                            J’ai lu et j’accepte les <a class="linkRules" href="https://www.ontheroadagame.fr/wp-content/uploads/2021/11/2021-11-02-Reglement-On-The-Road-a-Game-At-Home-Edition.pdf" class="woocommerce-terms-and-conditions-link" target="_blank">conditions générales</a>
                        </label>
                    </div>
                </fieldset>
                @error('session')
