    ...
    <div class="row containerArticles">
        @if ($articles->isEmpty())
            <div class="flex justify-content-center flex-column">
                <div>
                    <p class="price text-center">Aucun article pour le moment !</p>
                </div>
            </div>
        @else
            @foreach ($articles as $article)
                <div class="positionButton marginArticles col-lg-3 col-md-6 col-sm-12 containerPresentation">
                    <div class="flex flex-col justify-content-center">
                        <img width="280px" height=auto class="align-self-center imagePresentation"
                    ...
                    @auth
                        @if (Auth::user()->role->role === 'Admin Défis' or Auth::user()->role->role === 'Super Admin')
                            <div class="flex justify-content-around">
                                ...
                                <a class="btn buttonAdmin btn-success " href="{{ route('articles.edit', $article->id) }}">
                                    Modifier</a>
                                <a class="btn buttonAdmin seeAdmin" href="{{ route('articles.show', $article->id) }}">
                                    Voir</a>
                            </div>
                        @else
                            <div>
                                <a class="btn seeMore" href="{{ route('articles.show', $article->id) }}"> En savoir
                                    plus</a>
                            </div>
                        @endif
                    @else
