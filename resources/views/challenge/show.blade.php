            ...
            <div>
                        ...
                        <form action="{{ route('challenges.posts.store', $challenge->id) }}" method="post"
                            ...
                            @if ($challenge->editable == 0)
                                <p class="align-self-center text-center list-group-item list-group-item-danger"> <span
                                        class="warning">ATTENTION </span>, vous n'avez qu'une chance pour ce défi, une
                                    fois validée par l'administrateur vous ne pourrez pas le refaire !</p>
                            @else
                                <p class="align-self-center text-center list-group-item list-group-item-info">Vous avez
                                    plusieurs chance pour réaliser ce défi</p>
                            @endif
                            <br />
                            @error('file_path')
                ...
                @else
                    ...
                    @else
                        ...
                        <p class="text-center"> Pas de panique si votre vidéo ne s'affiche pas, <a
                                href="{{ $post->file_path }}" download>cliquez ici pour la télécharger</a></p>
                        <p class="text-center"> Si vous n'arrivez pas à la télécharger, il faut changer le format de la
                            vidéo !</p>
                            ...
                            <div>
                                @if ($post->state == 'pending')
                                    <p class="align-self-center text-center list-group-item list-group-item-info">Tant que
                                        le défi est en attente, vous pouvez toujours le supprimer et poster à nouveau</p>
                                @else
                                    <p class="list-group-item list-group-item-info"> <span class="warning">ATTENTION
                                        </span>, si vous supprimez votre post, vous perdrez vos points pour ce défi, quitte
                                        à en gagner moins avec un autre post !</p>
                                @endif
                            </div>
                        </div>
